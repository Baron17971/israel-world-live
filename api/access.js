...
function publicRoom(room){return{code:room.code,className:room.className||'',activeStage:room.activeStage,status:room.status,resultsVisible:room.resultsVisible,gameIndex:room.gameIndex||0,gameRevealed:Boolean(room.gameRevealed),version:room.version,lastActiveAt:room.lastActiveAt||room.updatedAt||room.createdAt};}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='create'){
      ...
      const room={code,teacherToken:crypto.randomBytes(24).toString('hex'),className:clean(body.className,60),activeStage:1,status:'closed',resultsVisible:false,gameIndex:0,gameRevealed:false,version:1,createdAt:now,updatedAt:now,lastActiveAt:now,pinSalt:p.salt,pinHash:p.hash,pinVersion:1};
      await saveRoom(room);
